...
net.Receive( "PORTALGUN_PICKUP_PROP", function()
	...
	if not IsValid( ent ) then
		--Drop it.
		if wep.PickupSound then
			wep.PickupSound:Stop()
			wep.PickupSound2:Stop()
			wep.PickupSoundStop = CreateSound( wep, "weapons/physcannon/portal2/hold_loop_stop.wav" )
			if snd_portal2:GetBool() then
				wep.PickupSoundStop:Play()
			end
			wep.PickupSound = nil
		end
		...
		--Pick it up.
		if not wep.PickupSound and CLIENT then
			wep.PickupSound = CreateSound( wep, "weapons/physcannon/hold_loop.wav" )
			wep.PickupSound2 = CreateSound( wep, "weapons/physcannon/portal2/hold_loop.wav" )
			if not snd_portal2:GetBool() then
				wep.PickupSound:Play()
			else
				wep.PickupSound2:Play()
			end
			wep.PickupSound:ChangeVolume( 0.25, 0 )
			wep.PickupSound2:ChangeVolume( 100, 0 )
		end

		-- wep.ViewModelOverride = true

		wep.ViewModelOverride = ClientsideModel(wep.ViewModel,RENDERGROUP_OPAQUE)
		wep.ViewModelOverride:SetPos(EyePos() - LocalPlayer():GetForward() * (wep.ViewModelFOV / 5))
		wep.ViewModelOverride:SetAngles(EyeAngles())
		wep.ViewModelOverride.AutomaticFrameAdvance = true
		wep.ViewModelOverride.startCarry = false
		-- wep.ViewModelOverride:SetParent(wep.Owner)
		function wep.ViewModelOverride.PreDraw(vm)
			vm:SetColor(Color(255,255,255))
			local oldorigin = EyePos() -- -EyeAngles():Forward()*10
			local pos, ang = wep:CalcViewModelView(vm,oldorigin,EyeAngles(),vm:GetPos(),vm:GetAngles())
			return pos, ang
		end
...
function SWEP:DrawPickupEffects(ent)
	...
	do
		...
		local increment3 = dir3:Length() / 2
		dir3:Normalize()
...
function SWEP:ViewModelDrawn(vm)
	...
	for k,v in pairs(VGravitySprites) do
		...
		if (owner:IsPlayer() and vm == owner:GetViewModel() and self.ViewModelFlip) then
			ang.r = -ang.r -- Fixes mirrored models
		end
...
function SWEP:DrawHUD()
	...
	local validMat = trace.MatType ~= 77 and trace.MatType ~= 88 and trace.MatType ~= 89
	local validBlu = true
	local validRed = true
	local hEnt = trace.Entity

	if hEnt:IsValid() and hEnt:GetClass() == "prop_portal" then
	...
	if not allsurfaces:GetBool() then
		if system:GetBool() then 
			if GetConVarNumber("portal_portalonly") >=2 then
				if orangeportal:IsValid() then
					Brack_left = "["
				else
					Brack_left = "{"
				end
			else
				if blueportal:IsValid() then
					Brack_left = "["
				else
					Brack_left = "{"
				end
