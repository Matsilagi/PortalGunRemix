...
function ENT:SetupPlayerClone(ply)
	if not ply.PortalClone then
		...
		ed:Spawn()
		ply.PortalClone = ed
	elseif ply.PortalClone:IsValid() then
...
function ENT:AcceptInput(name, activator, calle, data) --Map inputs (Seems to work..)
	if (name == "Fizzle") then
		self.Activated = false
		self:SetNWBool("Potal:Activated",false)
		self:CleanMeUp()
	elseif (name == "SetActivatedState") then
		print(name, data)
		self:SetActivatedState(true)
	else
		print("UNHANDLED ACCEPT INPUT", name, data)
	end
end
